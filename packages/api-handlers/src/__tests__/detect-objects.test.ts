...
describe('detectObjects', () => {
  ...
  it('should return mock data when USE_MOCK_DETECTION is true', async () => {
    ...
    const req = mockRequest({ imageUrl: mockImageUrl, modelId: mockModelId });
    const result = await detectObjects(req);
  ...
  it('should call HuggingFace API with correct parameters when not using mock', async () => {
    ...
    server.use(
      http.post(`https://api-inference.huggingface.co/models/${mockModelId}`, () => {
  ...
  it('should handle fetch errors', async () => {
    ...
    await expect(detectObjects(req)).rejects.toThrow();
